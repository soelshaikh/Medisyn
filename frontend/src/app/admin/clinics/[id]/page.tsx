...
export const metadata = { title: "Clinic Details | MediSyn Admin" };
...
export default async function AdminClinicDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!user || user.role !== "clinic") notFound();
  const [profile] = await db.select().from(clinicProfiles).where(eq(clinicProfiles.userId, userId)).limit(1);

  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-display text-2xl font-bold text-ink-900">{profile?.clinicName}</h1>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm lg:col-span-2">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            ...
            <div className="flex justify-between"><dt className="text-slate-500">Clinic phone</dt><dd className="text-ink-900">{profile?.clinicPhone}</dd></div>
            <div className="flex justify-between"><dt className="text-slate-500">Clinic address</dt><dd className="text-right text-ink-900">{profile?.clinicAddress}</dd></div>
